...
om_twist = 1 * (range(-0.5,0.5,length=n))*2#-twist_omega*zeta_rot / C * (rvecs[:,1] .- 0.5)
...
params = [lambda, h, n, omega, Cm, zeta_rot, C, Mmat, D1, D2, twist_omega]
...
@time global sol = solve(prob,alg_hints=[:stiff],saveat=tend/20,reltol=1e-12, abstol=1e-12)
