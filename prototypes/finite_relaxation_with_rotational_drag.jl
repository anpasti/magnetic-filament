...
θ = θmagic+0.08 # precession angle
...
params = [lambda, h, n, Cm, Cmrel, C, Mmat, D1, D2, D2_BC, zeta_rot]
...
function velocity!(du_arr,u_arr,params,t)
    ...
    du_arr[1:3*n]  = P*(total_force_arr)
...
for (i, u) = enumerate(sol.u)
    global rvecs_end = reshape(u[1:3*n], 3, n)'
    global om_twist_end = u[3*n+1:end]
end

c1 = rvecs_end[1,3]-rvecs_end[n,3]
c2 = -(rvecs_end[1,3]+rvecs_end[n,3])/2
l = range(-0.5,0.5,length=n)
om_twist_end_teor = -Cmrel/C * (  rvecs_end[:,3] + c1*l .+ c2  )
