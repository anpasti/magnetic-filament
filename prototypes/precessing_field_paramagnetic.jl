...
global n = 31#50#80 # number of points - each corresponds to an length element of a rod.
...
function velocity_prec!(dr_arr,r_arr,params,t)
    ...
    P = μ*P
    hvec = -normalize([cos(omega*t),sin(omega*t),0]) # rotating
    #hvec = -normalize([cos(omega*t),sin(omega*t),1/sqrt(2)]) # magic angle
    # hvec = -normalize([cos(omega*t),sin(omega*t),0.5/sqrt(2)]) # less than magic angle
    #hvec = -normalize([cos(omega*t),sin(omega*t),1.2/sqrt(2)]) # more than magic angle

    Mmat_Fparamag = make_Mmat_Fparamag(h,n,hvec, Cm)
    mvecs = make_paramagnetic_moments(rvecs,hvec,h,0.66,0.66/(1+0.66/2))
    fdip_vecs = make_dipole_force(rvecs,mvecs,6e-5*Cm,h)
    fdip_arr = reshape(fdip_vecs',3*n,1)
    dr_arr[:] = P*((Mmat + Mmat_Fparamag)*r_arr + fdip_arr)
end

# plot first frame
display(Plots.plot(rvecs[:,1],rvecs[:,2],aspect_ratio=:equal))
for period_n = 1:500 # run 5 periods, renormalize length every period
    global rvecs
    ...
    @time global sol = solve(prob,alg_hints=[:stiff],saveat=T/1,reltol=1e-12, abstol=1e-12)

    for (i, u) = enumerate(sol.u)
        local rvecs = reshape(u, 3, n)'
        local t = sol.t[i]
        local hvec = -normalize([sin(omega*t),cos(omega*t),1/sqrt(2)])
        #println(t/T)
